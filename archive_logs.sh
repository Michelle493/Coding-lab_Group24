...
if [ "$user_input" -eq 1 ]; then
	...
	archived_file=$ARCHIVED_LOGS/heart_rate/heart_rate_"$(date +'Y%-%m-%d_%h:%m:%s')".log

elif [ "$user_input" -eq 2 ]; then
        log_file=$ACTIVE_LOGS/temperature_log.log
        archived_file=$ARCHIVED_LOGS/temperature/temperature_"$(date +'Y%-%m-%d_%h:%m:%s')".log

elif [ "$user_input" -eq 3 ]; then
        log_file=$ACTIVE_LOGS/water_usage_log.log
        archived_file=$ARCHIVED_LOGS/water_usage/water_usage_"$(date +'Y%-%m-%d_%h:%m:%s')".log
