...
if [ "$user_input" -eq 1 ]; then
	...
	archived_file=$ARCHIVED_LOGS/heart_data_archive/heart_rate_"$TIMESTAMP".log

elif [ "$user_input" -eq 2 ]; then
        log_file=$ACTIVE_LOGS/temperature.log
        archived_file=$ARCHIVED_LOGS/temperature_data_archive/temperature_"$TIMESTAMP".log

elif [ "$user_input" -eq 3 ]; then
        log_file=$ACTIVE_LOGS/water_usage.log
        archived_file=$ARCHIVED_LOGS/water_usage_data_archive/water_usage_"$TIMESTAMP".log
